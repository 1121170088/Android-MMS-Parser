...
public class MmsDecoder
{
	...
	private boolean parseHeader() throws Exception
	{
		...
		switch (this.data.get(this.pos++))
		{
			case MESSAGE_TYPE: // OK
				...
				Log.v(TAG, "[MMS PARSER] MESSAGE_TYPE: " + this.message_type);
				break;
		
			case TRANSACTION_ID: // OK
				this.transaction_id = this.parseTextString();
				Log.v(TAG, "[MMS PARSER] TRANSACTION_ID: " + this.transaction_id);
				break;
			...
			case MMS_VERSION: // OK
				...
				Log.v(TAG, "[MMS PARSER] MMS_VERSION: " + this.mms_version);
				break;
				
			case TO: // OK
				this.to = this.parseEncodedStringValue();
				Log.v(TAG, "[MMS PARSER] TO: " + this.to);
				break;
				
			case SUBJECT: // OK
				this.subject = this.parseEncodedStringValue();
				Log.v(TAG, "[MMS PARSER] SUBJECT: " + this.subject);
				break;
				
			case FROM: // OK
				this.from = this.parseFromValue();
				Log.v(TAG, "[MMS PARSER] FROM: " + this.from);
				break;
				
			case MESSAGE_ID: // OK
				this.message_id = this.parseTextString();
				Log.v(TAG, "[MMS PARSER] MESSAGE_ID: " + this.message_id);
				break;
				
			case DATE: // OK
				this.date = this.parseLongInteger();
				Log.v(TAG, "[MMS PARSER] DATE: " + this.date);
				break;
				
			case DELIVERY_REPORT: // OK
				this.delivery_report = MMS_YES_NO.get(this.data.get(this.pos++));
				Log.v(TAG, "[MMS PARSER] DELIVERY_REPORT: " + this.delivery_report);
				break;
			...
			case CONTENT_TYPE: // OK
				...
				Log.v(TAG, "[MMS PARSER] CONTENT_TYPE: " + this.content_type);
				break;
		
			case BCC:
				this.bcc = this.parseEncodedStringValue();
				Log.v(TAG, "[MMS PARSER] BCC: " + this.bcc);
				break;
				
			case CC:
				this.cc = this.parseEncodedStringValue();
				Log.v(TAG, "[MMS PARSER] CC: " + this.cc);
				break;
				
			case CONTENT_LOCATION:
				this.content_location = this.parseTextString();
				Log.v(TAG, "[MMS PARSER] CONTENT_LOCATION: " + this.content_location);
				break;
			...
			case EXPIRY:
				...
				Log.v(TAG, "[MMS PARSER] EXPIRY: " + this.expiry);
				break;
				
			case MESSAGE_CLASS:
				this.message_class = this.parseMessageClassValue();
				Log.v(TAG, "[MMS PARSER] MESSAGE_CLASS: " + this.message_class);
				break;
	
			case MESSAGE_SIZE:
				this.message_size = this.parseLongInteger();
				Log.v(TAG, "[MMS PARSER] MESSAGE_SIZE: " + this.message_size);
				break;
				
			case PRIORITY:
				this.priority = MMS_PRIORITY.get(this.data.get(this.pos++));
				Log.v(TAG, "[MMS PARSER] PRIORITY: " + this.priority);
				break;
				
			case READ_REPLY:
				this.read_reply = MMS_YES_NO.get(this.data.get(this.pos++));
				Log.v(TAG, "[MMS PARSER] READ_REPLY: " + this.read_reply);
				break;
				
			case REPORT_ALLOWED:
				this.report_allowed = MMS_YES_NO.get(this.data.get(this.pos++));
				Log.v(TAG, "[MMS PARSER] REPORT_ALLOWED: " + this.report_allowed);
				break;
				
			case RESPONSE_STATUS:
				this.response_status = this.data.get(this.pos++);
				Log.v(TAG, "[MMS PARSER] RESPONSE_STATUS: " + this.response_status);
				break;
				
			case RESPONSE_TEXT:
				this.response_text = this.parseEncodedStringValue();
				Log.v(TAG, "[MMS PARSER] RESPONSE_TEXT: " + this.response_text);
				break;
				
			case SENDER_VISIBILITY:
				this.sender_visibility = MMS_YES_NO.get(this.data.get(this.pos++));
				Log.v(TAG, "[MMS PARSER] SENDER_VISIBILITY: " + this.sender_visibility);
				break;
				
			case STATUS:
				this.status = this.data.get(this.pos++);
				Log.v(TAG, "[MMS PARSER] STATUS: " + this.status);
				break;
				
			default:
				if (this.data.get(this.pos - 1) > 127)
				{
					Log.e(TAG, "[MMS PARSER] Unknown field (" + this.data.get(this.pos - 1) + ") for pos " + this.pos); 
				}
				else
				{
					this.pos --;
	...
	*---------------------------------------------------------------------*/
	private boolean parseParts()
	{
		...
		Log.v(TAG, "[MMS PARSER] Found " + count + " parts !");
		...
		for (int i=0; i<count; i++)
		{
			...
			Log.v(TAG, "[MMS PARSER] Part #" + i + "; Header len " + headerlen);
			Log.v(TAG, "[MMS PARSER] Part #" + i + "; Data len " + datalen);
			Log.v(TAG, "[MMS PARSER] Part #" + i + "; Content-type " + ctype);
	...
	*-------------------------------------------------------------------------------------------------*/
	private String parseFromValue() throws Exception
	{
		int len = this.parseValueLength();
		
		if (this.data.get(this.pos) == FROM_ADDRESS_PRESENT_TOKEN)
		{
			Log.v(TAG, "[MMS PARSER] Address-present-token found");
			this.pos ++;
			return this.parseEncodedStringValue();
		}
		else if (this.data.get(this.pos) == FROM_INSERT_ADDRESS_TOKEN)
		{
			Log.v(TAG, "[MMS PARSER] Insert-address-token found");
			this.pos ++;
			return "";
		}
		else
		{
			Log.v(TAG, "[MMS PARSER] No from token found, trying to skip the value field by jumping " + len + " bytes");
			this.pos += len;
		}
	...
	* *
	*------------------------------------------------------------------------*/
	private String parseEncodedStringValue() throws Exception
	{
		if (this.data.get(this.pos) <= 31)
		{
			this.parseValueLength();
			...
			if (charset.equals("utf-8")) raw = new String(raw.getBytes(), "UTF-8");
	...
	*--------------------------------------------------------------------------------*/
	private int parseValueLength()
	{
		...
		{
			Log.e(TAG, "[MMS PARSER] Short-length-octet (" + this.data.get(this.pos) + ") > 31 in Value-length at offset " + this.pos + "!");
			return 0;
		}
	}
	...
	*-------------------------------------------------------------*/
	private int parseIntegerValue()
	{
		...
		{
			Log.e(TAG, "[MMS PARSER] Not a IntegerValue field at pos " + this.pos);
			this.pos ++;
			return 0;
		}
	}
